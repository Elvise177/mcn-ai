    ...
    <div className="fade-up flex flex-col items-center">
      ...
      <div className="flex gap-6">
        <button
          disabled={busy}
          onClick={() => pick(true)}
          className="w-64 rounded-xl border border-accent-line bg-accent-soft p-6 text-left hover:opacity-90"
        >
        ...
        <button
          disabled={busy}
          onClick={() => pick(false)}
          className="w-64 rounded-xl border border-line bg-card p-6 text-left hover:bg-hover"
        >
      ...
      {onSkip && (
        <button onClick={onSkip} className="mt-8 text-sm text-muted hover:text-accent hover:underline">
          暂时跳过，之后可在「个人知识库」里建
        </button>
      )}
    </div>
