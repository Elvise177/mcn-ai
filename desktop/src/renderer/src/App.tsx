  ...
  const pick = async (k: 'local' | 'ai' | 'all'): Promise<void> => {
    if (k === 'all' && mode !== 'all') {
      const okd = await ui.confirm({
        title: '确认让敏感资料也上云？',
        message:
          // **弹窗正文是纯文本**（`whitespace-pre-line`，不过 markdown）：写 `**…**`
          // 出来就是两个星号摆在用户脸上（走查截图上一眼看出来的）。要强调就换措辞
          '人事档案、财务表、达人信息表这类被判为敏感的文件，会和普通文件一样上传到云端知识库。\n\n' +
    ...
    <label className="flex cursor-pointer items-start gap-2 border-b border-line pb-2 text-md">
      ...
      <span>
        ...
        <span className="block text-sm leading-5 text-muted">
          {/* JSX 文本不过 markdown：写 `**…**` 出来就是两个星号（同敏感档确认弹窗那处） */}
