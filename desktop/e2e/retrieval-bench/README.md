...
| 引用错位率（题｜条） | **尺子第二版（2026-09-05 拍板）**。回答里 `[[…]]` 能解析到库内文件的引用中：落在应命中集合内的天然相关；集合外的交给判分，判分拿到该文件的 frontmatter 摘要、正文开头 400 字和回答里引用它的那句话，判「与问题相关 / 不相关」并**必须写明依据**（`judge.citations[].basis`）。**不相关才算错位**。报两个数：有错位引用的题占比｜错位条数/解析到文件的条数。老结果没做过相关性判定的引用标「未判」，不算错也不算对。第一版口径「引用正确率 = 集合内占比」保留在 `summary.json` 的 `citationPrecision` 里，只用于回看 0.1.3 基线 | `vaultManager.resolveLink`（与产品校验引用同一函数）+ `judge.citations` |
