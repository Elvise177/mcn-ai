...
> 更新：2026-09-05（**检索基准 §0-新i；检索优化第一单 §0-新j**）｜ 范围：mcn-ai 产品线（桌面版 + 云端），不含 OMG 钉钉自动化定制项目（那是独立仓库 `~/Documents/AI/omg-dingtalk-automation`，有自己的 README 和交付文档）
...
| `PLAN-retrieval-wiki.md` | 检索优化第二单方案（实体卡 → wiki 主题页 + 引擎三小改；embedding 留第三单） | **待批** |
