...
> 当前阶段：MVP（M0–M4）+ **QA 回归修复大单四批全部完成** + **过程可见性已交付**；下一步是 A-3 达人卡进链
...
- **【A-3 + 图片大单 · 已做与仍缺（2026-08-18 记，未完工）】**：这一单只完成了 pipeline 侧的
  前半段，**验收远未开始**，接手前先看这张表，别按"已交付"去信任。
  - **已做且本地验过**：`02_convert` 抽图（docx/pptx，PDF 刻意不做）+ 降采样 + 豁免；
    `03_tag_llm` 三类实体字段并进现有那一次调用 + `is_contract` + checkpoint 改 `(path, rev)` 判重；
    `03b_tag_rules` 规则级实体（文件名认合作方、逐 Sheet 表头认达人，149 个，与老库 156 张同量级）；
    `images` 在 03/03b 显式保留（同 `sensitive` 被抹那次的坑）
  - **仍缺（一件都没开始）**：建卡器（TS 侧）、`07` 改读 layout + layout 的 `entities` 段（`30_实体/`）、
    `INBOX_FLOW` 的建卡阶段、`cloudSync` 敏感判定改读盘（**不改就是 A-8 回归**，见
    `docs/PLAN-entity-image.md` §0-3）、`mcnai-asset` 协议（**没有它，抽出来的图在应用里渲染不出来**）、
    附件直供、`render_pptx` 原生 chart + `spec` 补 `xlsxwriter`（**不补冻结形态必挂**，见 §0-2）
  - **未跑**：pipeline 未冻结（`resources/pipeline/mcn-ingest` 仍是 2026-08-17 那份，本单改动一个字节都没进去）、
    Maggie 全量重跑未跑、**Q4/Q7 未跑**、走查未跑。所以**双链仍是 2 条**，实体卡对检索的贡献
    目前**没有任何数据**，将来有了也要标注"待真实使用积累后复评"
