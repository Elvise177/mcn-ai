...
- **① 打标摘要**（pkb-pipeline `86bd7e4`）：`summary` 从「一句话摘要，30字内」改成
  ...
  **副作用如实记：bench 用的是已打过标的库，所以这一改在这一轮 bench 上数字不会动**，
  收益要等新入库的文档
- **② 跨文档答法**（规则 3d）：读完先**逐份**写这一份提供了什么要点，每份至少一行，过完再综合。
  针对 §8 归的那一层短板——"文件都读到了却答得浅"（跨文档要点覆盖 63%、核心覆盖 57%，
  6/10 题判分是"部分覆盖"）：一上来就揉成一段总结，必然是记得住的那两份说了算
...
**必须更正的一条**：§8b 那轮（只跑跨文档 10 题）测到规则 3d 让核心覆盖 57% → 68%，
**全量复跑没有重现**（56%，§7b 是 57%）。三轮 57 / 68 / 56，**68 是离群值**。
**规则 3d 目前没有可重现的收益**，代价是跨文档中位耗时 +8s。留不留由用户定（改回只动 desktop 一个文件，不用重新冻结 pipeline）。
