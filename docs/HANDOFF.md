...
> 更新：2026-09-06（**检索基准 §0-新i；第一单 §0-新j；第二单关闭 §0-新k；第三单方案 §0-新l；第三单落地 + channel 定案 §0-新m；第四单 入库链路 × 语义索引 §0-新n**）｜ 范围：mcn-ai 产品线（桌面版 + 云端），不含 OMG 钉钉自动化定制项目（那是独立仓库 `~/Documents/AI/omg-dingtalk-automation`，有自己的 README 和交付文档）
...
**为什么要做**：语义索引在此之前是 vault watcher 的副产品——入库落完文件，watcher 逐篇 upsert 去补。
一批 100 篇就是 100 次单篇推理 + 100 次全量落盘（`persist()` 每次重写整个 `.bin`），
**而进度条对它一无所知**：界面写着"投递箱处理完成"，索引其实还在后台慢慢补，
用户这时候提问拿到的是关键词档。

**做了什么（五条，都在 `787f465` 里）**

| # | 落点 | 要点 |
|---|---|---|
| 1 | `config/stages.ts` · `inbox/orchestrator.ts` | 入库尾段新增一格 **`embed_index`「建立语义索引」**，`INBOX_STAGES` 8 → **9 格**，位置在建卡之后、上云之前（前者：实体卡也是语料；后者：不该被"网不好的上云"挡着）。判据是**集合差 + 语料哈希，不是 mtime**。**扫盘取现状**，不用内存里那份 `notes`（watcher 去抖 800 ms，刚写完的卡还没进内存 → 整批漏掉）。60 秒墙钟预算，超了报"剩余在后台继续"往下走 |
| 2 | `tasks/types.ts` · `pages/VaultPage.tsx` · `smoke:steps` | 阶段总数的下游一起同步（6→7→8→9 的老教训）：`VaultPage` 写死的兜底 `total: 6` 改成从阶段表取；`smoke:steps` +5 条（它在流程里 / 位置对 / 标签与序号都从阶段表来 / 跑满 = 分母）。走查那条完成态判据本来就读进度条自报的 done/total，不用改 |
| 3 | `vault/embed-index.ts` · `smoke:embed` · `a1-enqueue` | 索引同步剔除（删除 / 移出库 / 重命名）、换库随库切换、索引文件不上云，**全部落成断言**（下面单列） |
| 4 | `App.tsx` · `ipc.ts` · `lib/diagnostics.ts` | 设置页「语义检索」区加一颗**「重建索引」**（整个丢掉重算）+ building 那一档补百分比；诊断导出新增「语义索引」一行（状态/篇数/模型/耗时/**失败原因**/合并方式） |
| 5 | `system-prompt.ts` 规则 3d ／ pkb-pipeline `taxonomy.py` | 两处 prompt，不改引擎（下面单列） |

**这一单撞出来的两个真 bug（都不是它引入的，是它的断言撞出来的）**

- **`persist()` 并发抢同一个 `.tmp`**。落盘是 write-tmp → rename 两步，调用它的有三条路（建索引末尾、
  单篇 upsert、单篇 remove，后两条还是 `void` 出去的）。并发时先跑完的把 tmp 改名走了，
  后一个 rename 直接 `ENOENT`——现场是"删一篇之后紧接着一次批量同步，整个同步报失败"，
  而那条 `void` 会把它变成一次**静默的 unhandled rejection**。修：所有动索引目录的活排进一条串行队列，
  tmp 名再带 pid+序号
- **`smoke:guards` 的【9】写在 `process.exit()` 后面，从来没跑过**。第三单立的"索引文件永不进上传清单"
  那三行是彻头彻尾的死代码，而 smoke:guards 一直报「✅ 全部通过」。**与 `computeProgress` 那条
  `label === '智能打标'` 死判据同一个病：绿色不等于跑过。** 挪回 exit 之前，并把目录名从
  `embedIndexDir()` 算出来，不再抄 `.mcnai`
- 顺带一条防患：`open()` 是"立即返回、后台接着算"的，换库时旧库那一轮 build 还在 batch 循环里，
  而 `setEntry` 写的是已经属于新库的 `meta`——**上一个库的向量会落进新库的索引**。
  加了换库代号 `gen`，每批 + 每次推理后各验一次

**断言（零 LLM、零网络）**

- `smoke:embed` 新增【索引生命周期】：6 篇合成笔记 + 两个临时库 —— 语义命中（零共词问句 top-3）／
  集合差只重算改过的那篇／内容没变一篇不算／删除后向量与盘上的 `.bin` 都不许留／`remove` 幂等／
  重命名旧名剔除新名入索引／**换库后上一个库的笔记一条都不许冒出来**／A 库索引文件原地留着
- `a1-enqueue` 第 ⑥ 节（真跑完一轮入库后）：`embed_index` 阶段事件真的出现过、
  **索引条数 == 文件树笔记数**（两侧各走各的路）、状态 ready 无失败原因，再删一篇验向量跟着掉。
  实测 **272 条 == 272 篇，本批新算 271 篇 3.1 秒**；删一篇 → 271
- 它现在也认 `MCNAI_APP_BIN`（同 `fresh-install`）：**发版前用它验打包形态**——语义索引这一格
  加载的是 onnxruntime-node 原生模块，开发目录跑通不能证明包里跑得起来（RELEASE.md §B 第 4 步已写）

**两处 prompt（不改引擎）**

- **① 打标摘要**（pkb-pipeline `86bd7e4`）：`summary` 从「一句话摘要，30字内」改成
  「先说这是什么，**再说这份文档能回答什么问题**」。现场是 bench 里的 J-S2《公域营销行动指南》——
  摘要写"明确目标、分工与复盘标准"，问法是「找博主 签约」，零共词，本地 embedding 也够不着（#99）。
  黄金母本随之更新（自 0.2.0 批 2 立起来以来**唯一一次有意的改动**，两边头注释都记了是哪一次、为什么）。
  **`SCHEMA_REV` 故意没 +1**：老库不重打标（+1 = 每个客户整库重打，真金白银）。
  **副作用如实记：bench 用的是已打过标的库，所以这一改在这一轮 bench 上数字不会动**，
  收益要等新入库的文档
- ~~**② 跨文档答法**（规则 3d）：读完先逐份写这一份提供了什么要点，过完再综合~~
  **已删（2026-09-06 当天）**：只跑跨文档 10 题那轮测到 +10pp，全 45 题复跑没重现（见下方门禁一节）。
  发版里不带它，agent 行为 = §7b 已实测配置

**顺手补上的一道闸**：`smoke:pipeline` 新增第 8 节，拿**冻结产物**的 `--print-prompt` 逐字节比黄金母本。
在此之前"打标口径没漂"只有源码侧守着，而客户机上跑的是冻结二进制——
「改了 pkb-pipeline 但忘了重新冻结 / 冻结了但忘了拷进 `resources/pipeline/`」这一整类事故
**对所有测试都是隐形的**（那个目录是 gitignored 的，git diff 也看不出来），
正是第 2 批 `03b` 漏进 PyInstaller `datas` 那次的形态。

**已知的小瑕疵（不修）**：升级到 0.1.4 之后，投递箱面板里"上次那一轮"的结果会显示成 `8/9`——
落盘的是旧版本那一轮的阶段列表，里面没有 `embed_index`。下一轮入库跑完就正常了，纯观感、自愈。

**验收**：`npm run verify` **16/16 全绿**（零花费，549 秒）；走查截图逐张看过
（`09-投递箱-完成后` 的进度条已是 `3/9`、新增 `71b-语义检索-重建索引完成`）。

**bench 跨文档 10 题复跑（`RETRIEVAL-BENCH.md` §8b，结果目录 `runs/opt4-prompt-multi`）**

| 指标 | §7b 基线 | 本轮 | 判断 |
|---|---|---|---|
| **核心要点覆盖** | 57% | **68%（+10pp）** | 规则 3d 的目标，达到了 |
| 读到率 | 68% | 71%（+3pp） | 同向 |
| 要点覆盖 | 63% | 64%（+1pp） | 基本没动 |
| 召回命中 | 84% | 78%（−7pp） | **不是 3d 造成的**，见下 |
| 全命中 | 50% | 40%（−10pp） | 同上 |
| 引用错位 | 0%｜0/30 | 0%｜0/35 | 守住 |
| 中位耗时 | 64.0s | 72.2s（+8.2s） | 逐份列要点，回答变长 |

**召回那两格掉的是哪两题，逐题查过（规则 3d 不碰检索层，掉的是模型自己换了检索词）**：

- **J-X2**：基线搜「年度目标」→《商务中台-2026年度目标管理总表》进前 10；本轮搜
  **「年度目标 高中低标」**（多了一个词）→ AND 更严，总表直接落空，3→2 变成 3→1。
  这是引擎层「n 词 AND」的老病（§6 的 n−1 档试过、未达标已关闭），与 3d 无关
- **J-X6**：基线搜「年中复盘 人效」「8月 战略调整」；本轮搜「人效」「待办清单」「战略调整」
  ——三次都没搭到能把《2026年中复盘会议纪要》捞上来的那组词。**还是那一份长表**（§8 归的第一号短板）
- 其余 8 题**缺的文件与基线一模一样**（三层漏斗 / 总结 / 会议纪要 ×2），一份没多一份没少

所以这一轮的读法是：**生成层那格（核心覆盖 +10pp）确实动了；检索层那两格是同一批老短板在
"模型这次换了个说法"下的重新暴露，n=10 时一题就是 10pp**（§9 记过：验证集每类 3-4 题时单题波动 ±25-33pp）。
要把它从噪音里摘干净，只能扩题或同题多跑，两者都要另花钱，**没做**。

**记账（如实）**：对话 ¥3.66 + 判分 ¥0.84 = **¥4.50**，跑前用户给的口径是「¥3 内」，**超 50%**。
两处估低了：① 跑前脚本按 ¥0.20/题估（那是关键词题的均价），跨文档题历来 ¥0.30–0.44；
② 规则 3d 让回答变长，判分的输入也跟着变长，判分从估的 ¥0.20 变成 ¥0.84。
中途在 M-X1 出来时（¥0.379/题）报过一次"预计 ¥3.8–4.2"。**累计 ¥66.70 → ¥71.20**。

**发版前门禁（用户 2026-09-06 加的两道，`RETRIEVAL-BENCH.md` §8c）**

| 门 | 结果 |
|---|---|
| ① 全 45 题 bench，四类召回/全命中/错位不低于 §7b（±一题） | **过**。合计 92%/83% 与 §7b 持平；全轮只有两题变了：**失 J-S2**（模型换检索词，两条查询 total=0）、**得 J-P4**（长表排名抖动自己回来了）；错位一题是 J-S4 多引了一份不支撑该结论的文件。¥13.90（口径 ¥14 内），86 分钟 |
| ② 10–15 篇文档用新 summary 口径真跑打标，人工看 | **过**。12 篇全部 AI 打标，¥0.1375（口径 ¥0.5 内）。J-S2 那篇新摘要写出了「获客建联并**签约**」，M-S5 那篇写出了「怎么让学员10天内发出第一条视频」。**边界**：《总结》的新摘要仍没有「星母计划」四字——正文里本来就没有，模型不能造专名，那一类还得靠语义通道 |

**零花费实验（新摘要值多少名次）**：把 bench 库副本里 J-S2 那份文件的 `summary` 换成真跑出来的新摘要
（只改这一行），`retrieval-tune` probe 比名次：「签约 达人 博主」10 → **8**、「签约 博主」10 → **7**、
「获客 建联」3 → **2**；「达人 招募 渠道」仍然无。
**新摘要给了 2–3 个名次的余量（从卡在第 10 格变成稳在前 8），但救不了 total=0 的那两条查询**——
J-S2 这次失手的主因是 **n 词 AND 一票返回 0**，摘要口径是缓解不是解药。

**必须更正的一条 → 规则 3d 已删掉**：§8b 那轮（只跑跨文档 10 题）测到规则 3d 让核心覆盖 57% → 68%，
**全量复跑没有重现**（56%，§7b 是 57%）。三轮 57 / 68 / 56，**68 是离群值**。
没有可重现的收益、却有 +8s 的代价，**用户 2026-09-06 拍板去掉再发版**——
删掉之后 agent 行为回到 §7b 那个**已经完整实测过的配置**（92%/83%），不需要为它再跑一轮 bench。
`system-prompt.ts` 一个文件的事，不涉及 pipeline 重新冻结。
...
1. **《2026年中复盘会议纪要》长表压排名**——§8 里 9 份"没摆到面前"的文件有 3 份栽在这一份上，
   语义通道对它给的是人名卡。候选：实体卡语料在语义通道降权/单列、或长表按段落切索引。
   **先算清有几份文件受益再动手**
2. **n 词 AND**（J-X2 这次的现场）：第二单的 n−1 档因为"抢在模糊档之前且回得少"被关掉，
   判据本身没错，是档位顺序错了。重开要重新定顺序，且要跑回归
3. **打标 summary 新口径的实效没验**：老库不重打标，这一轮 bench 看不见它。
   要验得拿一小批文档真跑一次新打标再比，**是一笔单独的钱**

---

## 0-新m. 检索优化第三单落地（本地 embedding 语义通道，2026-09-05/06，未发版）——**达标，合并方式定 channel**

> 数字与归因 `docs/RETRIEVAL-BENCH.md` §7；实现与方案出入 `docs/PLAN-retrieval-embedding.md` §10。

**决策（写死，别再拍脑袋）**
- **合并方式 = channel：关键词前 7 + 语义前 3**（`store.semanticMerge` 默认 `'channel'`；rrf 代码保留只供 bench）。同 18 题：channel 召回 98% / 全命中 94%，rrf 89% / 69%，第一单 82% / 63%。RRF 的病是方案预言的那条：语义噪音与精确命中同台——「班委」一查，4 份绩效档案把真命中挤出前 10。配额 7+3 一轮达标，6+4 没试
- **运行时 = 直接 onnxruntime-node ≥1.29 + 自写 WordPiece**，不经 transformers.js（硬依赖 sharp 多一片 Mach-O）。1.21 在 Electron 43 退出时 SIGABRT（`mutex lock failed`），`smoke:embed` 退出码守着
- **模型随包**：`resources/models/bge-small-zh-v1.5/`（gitignored；`scripts/fetch-models.mjs` 拉 + sha256 校验，**打包前必跑**，RELEASE.md 已写）。dmg 230 → 275 MB
- 索引 `<库>/.mcnai/embeddings/`，敏感文件照常进索引；**不上云**：`isCloudSyncSkipped` 跳过点开头目录 + `smoke:guards`【9】
- 设置页「知识库」卡 → 「语义检索」区：开关 + 状态行（已建 N 篇 / 建索引中 M/N / 不可用：原因——已退回关键词 / 已关闭）；走查新增断言（篇数从 IPC 取、开关真点、命中测试）

**尺子修正（重要）**：bench 重放原来只走关键词通道，语义通道摆出来的条目不计入"摆到面前"——两轮跑完才发现。合并逻辑抽成 `agent/merge.ts` 纯函数两侧共用；加 `--rereplay`（零 LLM）重算口径。**以后改检索出口，先问一句"重放还与产品同一条路吗"。**

**没救回的**：J-X5《会议纪要》（长表压排名，语义通道给的是人名卡）；J-S2《公域营销》（#99，摘要增强另立一单）。跨文档要点覆盖 −13pp 是生成层波动（两种合并方式都掉、文件都读到了）。

**全 45 题一轮已跑（2026-09-06，`RETRIEVAL-BENCH.md` §7b）**：召回 92% / 全命中 83% / 语义题 100% / 引用错位 0 / 陷阱 5/5；敏感区 −10pp 是 J-P4 一题（《会议纪要》长表排名老病，离线重放证明与 7+3 配额无关）。**检索优化这条线收口**，总结与短板归层见 `RETRIEVAL-BENCH.md` §8（对客户讲准确率的底稿）。
**记账**：18 题两模式 ¥11.45 + 全 45 题 ¥13.87（报 ¥12–14）= 本单 ¥25.32；**累计 ¥66.70**。
~~**不发版**（用户 2026-09-06）：等下面「入库链路 × 语义索引」一单做完一起发~~ → **两单一起出 0.1.4**（见 §0-新n）。

**连带另开一单（用户 2026-09-06 拍板，本单不混）——「入库链路 × 语义索引」→ 已做完，见 §0-新n（提交 `787f465`）**：
1. 语义索引作为**入库尾部阶段**：现在是 watcher 事件驱动的 upsert（`vault/index.ts` → `embedIndex.upsert`），入库 run-end 批量落文件时靠 watcher 去抖逐篇补，进度条不知道它——要变成 `INBOX_FLOW` 里可见的一格（打标之后、上云之前），`computeInboxProgress` 加对应阶段
2. **删除同步**：文件删除 / 移动 / 重命名 → 索引条目同步删（现在 unlink 走 `embedIndex.remove`，move 是 unlink+add，重命名后向量重算一次；要在 smoke 里断言"删了的文件不会再从语义通道冒出来"）
3. 换库 / 建库向导 / 清库 时索引目录的处置；`.mcnai/embeddings` 进诊断导出
验收走 `e2e/a1-enqueue.mjs` 那条投递链路，不并回主走查（desktop/CLAUDE.md 的规矩）。

---

## 0-新l. 检索优化第三单方案（本地 embedding，2026-09-05）——已批、已落地（见 §0-新m）

> `docs/PLAN-retrieval-embedding.md`。选型有本机实测：5 个候选模型在两库 971 篇「标题。摘要」语料上跑 8 道词汇鸿沟题——**bge-small-zh-v1.5 q8**（24 MB、加载 1.5 s、建索引 5 s、峰值 RSS 496 MB）8/10 目标进前 10，bge-m3（570 MB、30 s、76 s、1.1 GB）只有 5/10 且更差，nomic 中文崩。M 系 8G / Intel 8G 两档是估算，装机后补实测。
> 关键取舍：不用向量库（1000 篇 × 512 维 = 1.9 MB，平面文件 + 暴力余弦）；语料只到摘要级；合并用**分通道各取 N**（关键词 7 + 语义 3 带标记）而非 RRF，bench 两种都跑；模型随包分发不联网；降级三档（可用 / 仅索引卡与摘要 / 纯关键词）；敏感文档进本地索引、索引文件不上云（walk 显式跳过 `.mcnai/` + smoke 断言）。
> 验收：验证集 15 + 词汇鸿沟专项 8（重叠 4）共 19 题，两种合并方式各一轮 ≈ ¥14，跑前报数。已知救不回的：J-S2《公域营销行动指南》（bge-small #99，摘要与问法不搭，留给"摘要增强"另立一单）。

**尺子记账（2026-09-05 晚）**：基线语义题相关性补判 ¥0.59 → 基线 v2 错位率 0%｜0/17（第一单 10%｜1/21）。**累计 ¥41.38**。
**roadmap**：验证集扩到 30 题（每类 6 + 陷阱 6，加题不删题）；**对外只引全 45 题的数字**，验证集只作内部过拟合判断。

---

## 0-新k. 检索优化第二单试跑（引擎层，2026-09-05）——未达标，默认关闭

> 方案 `docs/PLAN-retrieval-wiki.md`（已批）；结果与三条归因在 `docs/RETRIEVAL-BENCH.md` §6。

做了什么：`vault/wiki-pages.ts` 主题页生成器（目录主题 + 标签主题，摘要区复用已有 summary/tags，敏感继承与建卡器同一套、`smoke:cards` +14 条）；索引加 `dir` 字段、排名参数可配（`search-worker.ts` `configure`）、n−1 AND 档（带 `dropped` 告知）；`retrieval-tune.ts` 零花费离线回归；bench 分调参集 30 / 验证集 15（`split`），对外只报验证集。
**结果**：验证集跨文档全命中 0%（目标 ≥70%），语义召回 67%（−0），全 45 题跨文档全命中 40 → 40%、读到率 71 → 61%。T-1 守门 ✓。
**为什么**：① 主题页进了 25 题前 10，模型只 Read 4 次，反把原文挤出位次；② n−1 档抢在模糊档之前且回得少（M-S3 39 条 → 1 条）；③ 排名参数 8 组离线回归一格没动。
**处置**：代码保留、默认关闭（`MCNAI_WIKI_PAGES=1` 开主题页；`relaxed: 1` 开 n−1 档），dir 字段与 title 6 权重保留；产品行为等于第一单。下一步只剩两个杠杆：让模型认得主题页（工具输出 📚 前缀 + 规则 3c 一句话，重跑跨文档 10 题 ≈¥3.5）、摘要区本地 embedding。
**记账**：¥11.11 + 判分 ¥2.70 = ¥13.81（跑前报 ≈¥15）。**累计 ¥40.79**。
**尺子**：v2 引用错位率（10%｜1/21 为对外口径）；验证集每类 3-4 题、单题波动 ±25-33pp，报数带样本量。

---

## 0-新j. 检索优化第一单（agent 策略层，2026-09-05，未发版）

> 依据 §0-新i 的基线；数字与逐题归因在 `docs/RETRIEVAL-BENCH.md` §5。**没动检索引擎。**

| 改动 | 落点 |
|---|---|
| 多子部分问题逐子部分多轮检索（新规则 3c；规则 9 上限单点 3 次 / 汇总 6 次，连续一轮无新文件就停） | `agent/system-prompt.ts` |
| `search_knowledge` 单次返回 6 → 10 条、按笔记去重、超出时告知"共 N 条只列前 10"；相近结果附"先 Read/Grep 验证再引" | `agent/index.ts`（`SEARCH_SHOWN_LIMIT` 导出给基准重放用） |
| 回答里「库里有据（带引用）」与「推断：（不带引用）」分段（新规则 10c）；相近结果引用前必须验证（规则 1b） | `agent/system-prompt.ts` |
| B-6 引用校验把 Grep/Glob 结果里的文件也算"看过"（`notePathsFromScanResult`，认三种输出形态，路径含空格也认） | `agent/steps.ts` · `agent/index.ts`；`smoke:steps` +5 |

**结果（对照基线）**：语义题召回 70% → 90%、跨文档召回 68% → 81%、读到率 54% → 71%、要点覆盖 55% → 64%；敏感区核心覆盖 80% → 90%、引用 91% → 100%；陷阱拒答 5/5 不变；四类召回无一下降。
**两项本单指标没动**：跨文档全命中 40% → 40%（剩下每题差的那一份是引擎够不着的：泛标题《总结》、AND 不满足的《金字塔方案》、排第 15/19 位的《会议纪要》），语义引用正确率 41% → 43%（错位题 3 → 1，但模型多引了相关真实文件，按现口径全算错——建议把口径改成"引用错位率"，见 RETRIEVAL-BENCH §5）。
代价：中位耗时各类 +8～15s，对话实花 ¥11.66（+¥1.55），判分 ¥2.46，合计 ¥14.12（跑前报 ¥11.5–12.5，超 13%）。

**尺子第二版（2026-09-05 用户拍板）**：语义题指标改为「引用错位率（题｜条）」——集合外引用由判分看文件摘要/正文开头/引用句判相关性并写依据（`judge.citations[].basis`），不相关才算错；只重判本轮 10 道语义题（¥0.67）：**错位率 10%｜1/21**（J-S3 引《积分福利规则》答报销）。口径抓"引了无关文件"，抓不到"正确文件没找到拿相关文档硬答"（M-S5），后者看召回与覆盖。基线语义题未补判（还需 ¥0.67，下次跑基线顺带）。
**预算账**：本单跑前报 ¥11.5–12.5，实花对话 ¥11.66 + 判分 ¥2.46 + 补判 ¥0.67 = **¥14.79，超 13%（按对话+首轮判分 ¥14.12 算）**；基线轮 ¥12.19。累计 ¥26.98。
**第二单方案已出待批**：`docs/PLAN-retrieval-wiki.md`——实体卡 → wiki 主题页（目录主题 + 标签主题，摘要区复用已有 summary/tags，零 LLM）+ 引擎三小改（索引加 `path` 字段、n−1 AND 档、title boost/BM25 `b` 用 bench 逐题回归定），词汇鸿沟（M-S5 类）留给第三单的摘要区本地 embedding。

挂账：① 引用解析器两处老边角——文件名带尾随空格的笔记（《2026-各业务线年度目标拆解 .md》）与同名笔记（《100天晋升计划》两篇）会被标"存疑"，与本单无关；② 引擎层第二单：标题命中提权 + 同义/摘要匹配（跨文档全命中与 M-S5 的唯一出路）。
...
## 0-新h. PLAN-v2 批 5「可测性与账本」（2026-09-04 **整批完成**，未发版，随下一版出）

> 方案 `docs/PLAN-v2.md` 批 5 全部五项：**R8 + S2 + 对账**（跨 pkb-pipeline）、**R9 / F11 / R16**。
> 真实调用合计 **¥0.23**（原估 ≈¥1），其余全部零花费。
...
### 顺手逮到的三个既有缺陷（都不在单子上）　—— **三条都已修（2026-09-04 当天）**
...
**三条的修法（第 2、3 条 2026-09-04 当天补做，跨 pkb-pipeline）**：

| # | 修法 | 落点 |
|---|---|---|
| 1 | 判据先起个名字再用，语句不以 `/` 打头 | `scripts/smoke-pipeline.mjs` |
| 2 | 支持格式收成一份真相：`02_convert.SUPPORTED_EXT = CONVERTERS ∪ DIRECT_COPY_EXT`，`cli.py` 与 `taxonomy.py --supported-ext` 都读它，不再各拼各的 | `02_convert.py` · `cli.py` · `taxonomy.py` |
| 3 | 本批清单改 **集合差 ∪ mtime 新**：集合差认直拷进来的新笔记，mtime 认同名重投被覆盖的 | `cli.py` 的 `_batch_files()` |
| 3b | 顺带理顺「假 ok」：空批次的判断挪到 `run_stage` **外面**，不再 skipped 后面再补一条 ok | `cli.py` |

`3b` 桌面端也一起硬化了（`judgeTagRan`：**只要出现过 skipped 就算没跑**，哪怕后面跟着 ok）——
pipeline 修了，但客户机上的老产物还会打那条假 ok，判据得自己站得住。

**这几条断言是先看着它红才留下的**：把修复前的源码 stash 掉重新冻结跑了一遍，
`smoke:pipeline` 第 7 节红了 6 条（原件进 `.failed`、误报「格式不支持」、
`.md` 只拿到规则打标、空批次补假 ok…），修回来才全绿。
第 7 节的样本 mtime 特意**拨老一小时**：脚本是现写现跑，文件天生"很新"，
不拨老的话 mtime 那条判据会碰巧成立，把"集合差认新文件"这件事整个掩盖过去。
...
| `e2e/fresh-install.mjs` **已按两步写法改，但没跑过** | 建库要先点 `wizard-create` 再点模板卡（0.2.0 之后是两步），原来只点了第一颗、卡在"投递箱未就绪"。改法照抄 walkthrough 里已验证的那段，但 fresh-install 是 L4（真实调用 ≈¥0.4），本轮没跑——**下次跑 `verify --paid` 时留意它** |


### R9 · agent result 判据抽成纯函数（`agent/result.ts`）

`judgeResult(msg, expectedModel)` → `{kind, error, models, degraded, resolvedModel, billable, sessionUsable}`。
这段判据只有**真实调用**才走得到（要上游真的 401、真的把模型换掉、真的在
`subtype:'success'` 里塞 `is_error:true`），留在 `runOnce` 的三百行里就是没人测——
`computeProgress` 那行死判据就是这么在线上活了几个月的。

**四条判据方向各不相同，刻意不合并**（这也是它值得单开一个文件的原因）：

| 用途 | 判据 | 为什么不能跟别人共用 |
|---|---|---|
| 显示 | 只认 `is_error` | `api_error_status` 搬到显示上会把一条**真回答**判成错误、连正文一起丢掉 |
| 记账 | `success && !is_error && !api_error_status && 真产生过 token` | 少记一条代价为零；B-2 实测余额耗尽那轮 8 个失败请求全被记进 jsonl |
| 会话续接 | `!is_error && !api_error_status` | 失败轮的 `session_id` 很可能没在 CLI 侧落过盘，留着会让整个对话此后每次都报「No conversation found」 |
| 降级 | 主模型在不在 `modelUsage` 里 | 不能取 `models[0]`——一轮里主模型与轻量模型同时出现，顺序由服务端给 |

`smoke:steps` +17 条 fixture。写这批 fixture 时自己先栽了一次：`modelUsage` 给成
`{[PRO]: {}}`（没有 token），`billable` 当场变 false——因为 `tokensOf` 在 `usage` 与
`modelUsage` 并存时**优先取 modelUsage**（两边说的是同一批 token，求和会翻倍）。
已经补一条断言把这个偏好钉住。

### F11 · 连着进不去的文件停掉自动重跑（`inbox/attempts.ts`）

审计 Q10 那个循环：某阶段抛异常 → 后续 skip、**原件不归档**（还在投递箱）→ 重启后
watcher 以 `ignoreInitial:false` 重新拾起 → 整条链再跑一遍 → 再崩。**而打标是花钱的。**

判据 `judgeAttempts(prev, stillThere, reason, now)`：跑完还留在投递箱里的文件记一笔，
连着 **3** 轮就搬进 `.failed/` 并写一句"为什么不再自动重试"。
**「连续」当真**：中间成功过一次就清零——不清的话，今天失败两次、下个月再失败一次
会被判成"连续三次"，而它中间明明进去过。

**顺带补了一个从没成立过的解析**：读 `失败原因.txt` 原来按 `名字 —— 原因` split，
而盘上是 pipeline 写的「· 名字 / 缩进 原因：…」——两种破折号一个都不在文件里，
`reason` 恒为 undefined，失败清单里只有一串光秃秃的文件名。写的那一半 0.1.2 修过
（「`split(" (")[0]` 把原因整个扔掉」），读的这一半没跟上。
连带修了两处：清单里原因**被 `truncate` 截在「转换没有产…」上**（F11 那句三行的话一截等于没写，
改成文件名截断、原因换行），以及 `.failed/<日期>/` 用了 **UTC 日期**
（pipeline 用本地日期，UTC+8 的用户在凌晨 0–8 点会落进前一天的夹子，`latestFailedDir()` 只认最新那个）。

### R16 · 两处静默失效

**① search-worker 崩了就地重建。** 原来 `on('error')` 只 `console.error` 一行，
`exit` 压根没接——worker 被 OOM kill 时 pending 里的查询**永远不会 resolve**，
搜索框转圈转到天荒地老。现在：先放掉在等的查询（回空，`search` 的契约是"总会 resolve"）→
重建 worker → 拿 `lastDocs` 把索引重灌回去。**重建封顶 3 次**：一起来就崩的话
无上限重建就是每秒几十次的 spawn 循环，把主进程一起拖垮。

**② 库根不可访问 → Condition `vault-lost` + 顶条。** 库放在外接盘/网盘上被拔掉后，
watcher 从此不报事件，而应用一点反应都没有——文件树还画着旧快照、检索还在旧索引上命中，
点开笔记才报「找不到文件」，**而每一次写入都在往不存在的路径上写**。

两条实现上的取舍：
- **心跳是主力，事件只是提前触发**。macOS 上外接盘被拔时 fsevents 往往**直接不吭声**，
  只等 `error`/`unlinkDir` 的话那台机器上这条顶条永远不会出现——而那正是最该被接住的场景。
  开库挂 30 秒心跳（`MCNAI_E2E_VAULT_PROBE` 走查压到 0.8 秒），两个方向都归它管。
- **结论一律以磁盘探测为准**。chokidar 的 `error` 大多是单个文件的瞬时问题
  （正在被写、被别的程序锁着），见 error 就顶一条吓人的横幅比不报还坏。
  同理 `unlinkDir ''`：有些同步盘会"删了又立刻建回来"，事件是真的、状态却已经恢复。

恢复后**自己撤掉**，不用重启（同 Q11 离线重探的教训）。

### 批 5 后半段的验收

- `npm run verify` **15/15 全绿**
- `smoke:steps` +17（R9）、`smoke:guards` +30（F11 13 条 + 失败原因解析 5 条 + R16 12 条），全部零花费
- 走查 +2 张截图，与本批预估的 2 张一致：`73-失败件-清单带原因`、`74-知识库目录不可访问-顶条`。
  R16 那条用 **chmod 000** 造（同 42 步造笔记读失败），并断言**恢复权限后顶条自己消失**
- F11 的走查只断言得到「原因渲染得出来」，断言不到"我刚写的那个文件在列表里"：
  面板是 `failures.length ? failures : diskFailures`，而事件那份会随任务对象熬过 reload，
  有它在就轮不到读盘那份（**设计如此**）。盘上那条链由 IPC 断言守着，两边合起来才完整
...
| 项 | 值 |
|---|---|
| 版本 | 0.1.3（含 §0-新d 批 0+1、§0-新f 档位契约 v2、本节批 2+3） |
| dmg sha256 | `9b8e38dd66f246c7934106a20da28586935ee5561e5d948567de90fb9768e98f` |
| 签名公证 | `verify-signing.mjs` **10 条全过**（含"包内 110 个 Mach-O 全部已签"） |
| 更新源 | `https://samepage-updates.oss-cn-chengdu.aliyuncs.com/mac/`，线上已是 `version: 0.1.3` |
| 上传顺序 | zip → blockmap → dmg → **最后** `latest-mac.yml`（带 `Cache-Control: max-age=60`，已核） |

传 yml 之前**逐个核过三个包在线上的 `Content-Length` 与本地一致**——
先传 yml 的话，客户端会在包还没传完的窗口里读到新版本号然后 404。
硬约束（生产 `client-config` 须先是契约 v2）在发版前复核过：`contractVersion: 2`、两档都下发。

**注意**：`latest-mac.yml` 一上去就对全部客户生效。大头那台仍是 §0 里记的"发版后第一优先"
——她现在能收到这一版了，升级前记得先告诉她**别做批量重新入库**。
...
| `RETRIEVAL-BENCH.md` | 检索准确率基线（0.1.3，45 题）+ 第一单对照；尺子在 `desktop/e2e/retrieval-bench/` | 现行 |
| `PLAN-retrieval-wiki.md` | 检索优化第二单方案（wiki 主题页 + 引擎三小改）；已试跑未达标，代码默认关闭 | 记录 |
| `PLAN-retrieval-embedding.md` | 检索优化第三单方案：本地 embedding（bge-small-zh 选型实测、平面向量文件、分通道合并、降级三档、随包分发） | **待批** |
