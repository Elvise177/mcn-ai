...
> **A-3 实体层与图片能力已上线（功能完整、冻结产物就位）**；下一步是补完 A-3 的三项真实调用验收
...
2. ~~**A-3 达人卡进链 + 图片能力**~~ 🔶 **2026-08-18 功能全部完成，验收差三步**：实体层（三类卡 +
   归一 + 敏感继承 + 合同枢纽）与图片能力（嵌图落盘/渲染、附件直供、原生 chart）已上线并冻结；
   双链 2 → **400**（旧库基线 352）。**但 E2E_CHAT 全量走查、make-ppt 真跑人工过目、Q4/Q7 复跑
   这三项真实调用验收一项都没跑**——详见 §3「未解决/未做」里的 A-3 条目，别按"已交付"信任。
...
- **【A-3 + 图片大单 · 已做与仍缺（2026-08-18 更新，功能全做完、验收还差三步）】**
  代码与冻结产物都到位了，**但三项真实调用验收没跑**，接手前先看清这张表再决定信任到哪一层。
  ...
  **仍未跑（三项，都要真实调用）**：
  - **E2E_CHAT 全量走查**：最后一次全绿停在 `f1b7302`，其后的 `0eb0384`（chart/withVault）
    与 `35b4099`（建卡器三修）**没有重跑过**
  - **make-ppt 真跑一次**（含附件图 + 库内嵌图 + 原生 chart）并人工过目——从没跑过
  - **Q4 / Q7 复跑**——从没跑过，**所以"实体卡对检索的贡献"目前没有任何数据**；
    将来跑出来也要标注「待真实使用积累后复评」（单次问答不足以证明检索改善）

  预算：本单已花 **¥1.02 / 预算 ¥8**（`docs/PLAN-entity-image.md` §4），剩余三项估 ¥3
