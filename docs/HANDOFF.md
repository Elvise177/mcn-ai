...
> 当前阶段：MVP（M0–M4）+ **QA 回归修复大单四批全部完成**；下一步是过程可见性

---

## 0. 当前状态与待办排序（2026-08-18 收官）

### 修复大单：四批已提交，**但第 3 批漏了 A-1**

用 Maggie 源数据在隔离环境重跑 + 批跑问答，抓到的问题分四批处理：
...
| 第 3 批 | `20aef24` | B-2 计价对齐真实账单、A-7 上云不再截断、B-6 引用校验、B-7 幽灵模型、B-5+T-02 错误中文化、`showCost` |

> ⚠️ **`PLAN-fix-batch.md` 里第 3 批的标题是「A-1 + P1 三项」，实际只做了 P1 那几项，
> A-1 没做**（2026-08-18 收官核实时发现）。`orchestrator.enqueue` 至今只做一层 `readdir`，
> 嵌套子目录仍然跳过、`n===0` 仍然静默。**A-1 严重度是「高」，已排进下面的待办第 2 位。**

**逐条状态**：

| 已修 | 未修 |
|---|---|
| B-1 高 · A-8 高 · A-2 高 · A-4 中 · B-2 中 · A-7 中 · B-3 低 · B-5 · B-6 · B-7 | **A-1 高**（第 3 批漏做）· A-3 中 · A-5/A-6 低 · B-4（已归档，由网关方案覆盖） |
...
1. **过程可见性**（下一单，已开工）
2. **A-1 整包拖入递归（补做，高）**：`enqueue` 递归时**必须保留相对子路径**（落位与
   `category/sub_category` 全靠源目录树，拍平等于砸掉 92/92 的落位一致率）；连带看
   `chokidar` 的 `depth:3`；补 `n===0` 的反馈。做法见 `PLAN-fix-batch.md` §A-1。
   它依赖 A-4 的失败呈现才完整，而 A-4 已在第 2 批落地，所以现在可以直接做
3. **A-3 达人卡进链 + 图片能力**：`08_table_to_cards` 接回链，立项时**一并**评估敏感笔记的关联怎么补（§3-7），不要各修各的
...
5. **【2026-08-17 QA 回归批次】**：用 Maggie 源数据在隔离环境重跑并批跑问答，抓到的问题清单见
   ...
   修复分四批完成，**逐条状态见 §0**（未修的只剩 **A-1（高，第 3 批漏做）**、A-3、A-5/A-6）。按严重度：
