...
> 更新：2026-08-18（品牌二期：产品更名「拉齐」）｜ 范围：mcn-ai 产品线（桌面版 + 云端），不含 OMG 钉钉自动化定制项目（那是独立仓库 `~/Documents/AI/omg-dingtalk-automation`，有自己的 README 和交付文档）
...
- **R-02 云端敏感存量**：测试账号 `mcnai-test-a` 的 private 层 **3019 → 0**，
  ...
  **大头账号只查未动**：private 3581 条里 **4 个文件 / 398 条切片**要处置
...
- **更名**：`productName` 不改，见 §4-27 的复议结论
...
20. **品牌视觉二期：更名「拉齐」＋ 新 logo ＋ 配色体系切换（2026-08-18）**。只动渲染层样式/文案 + 图标产物，
    ...
    - **已知未达 AA 的一处（如实记，不是遗漏）**：橙色**实心块上的白字**（发送键、笔记「保存」、
      ...
      （走查每轮真算，见下）。
    - **不改的东西（拍板，见 §4-27）**：`productName` / dmg 名 / 包名 / bundleId / userData 目录名 / 钉钉群消息文案。
...
27. **更名只改"看得见的"，`productName`/包名留给苹果签名那一单（2026-08-18 拍板）**。
    ...
    - **⚠️ 2026-08-19 复议结论：`productName` 仍然不改**（发布前自测后拍板）。
      虽然上面那条"会强制重登"的理由已被实测推翻（改它不动 userData 也不动 Keychain），
      但**收益太小、不值得在第一个正式签名版上引入变量**：显示层已经全是 SamePage，
      够用了。**代价记在这里**：dmg 文件名、.app 包名、Finder/Dock 名、
      macOS 菜单栏应用名**继续显示 `mcn-ai`**，与界面不一致，客户看得见。
      **真正的内部改名等"天然要求重登"的时机搭车**——当前最可能的是**网关切换那一单**
      （key 不再下发客户端，客户端配置本来就要重来一遍）。
