...
### 换签名的代价：实测结论与预期相反（2026-08-19）

**原先的预期是「换签名后 Keychain 会把它当成新应用，客户要重新登录一次」。
本机实测这条没有复现。**

做法：拿**本机真实 userData 整份副本**（它是被**旧的 ad-hoc 签名版**写下的），
让**新的 Developer ID 签名版**接管（`e2e/upgrade-path.mjs` +
`MCNAI_APP_BIN=release/mac-arm64/mcn-ai.app/…`）。七件事全绿：

| 断言 | 结果 |
|---|---|
| 10 个配置字段无损（共 24 个字段的 config） | ✅ |
| 落盘 vaultPath 沿用，不掉进建库向导 | ✅ |
| 会话历史条数不变 | ✅ |
| 两把 key 都还在 | ✅ |
| **登录态恢复，不要求重新登录** | ✅ **会话真解了密**（`auth.state()` 走 `getSession()` → supabase-js 存储适配器 → `safeStorage.decryptString`，不是只看密文在不在） |
| `tierMigrated=true` 不二次迁移 | ✅ |
| `pricing.rev` 不倒退 | ✅ |

也就是说：**新签名的包能直接读出旧 ad-hoc 签名版加密的数据，Keychain 没弹授权框、
没当成新应用。** 另跑的 `e2e/probe-safestorage.mjs` 对照也一致——新旧两个包
`encryptString`/`decryptString` 往返都是 0ms 且无提示（`encryptString` 必须先读出
Keychain 里的主密钥，读得到就说明没被拦）。

#### 但**不要**把这条当成她那台的保证

Keychain 条目的 ACL 是**每台机器自己的历史**——本机这条 `mcn-ai-desktop Safe Storage`
经历过多少次构建、有没有人点过「始终允许」，我查不到也无法复制到她那台。
所以上门/远程升级时按**可能要重登**准备：

1. **带上她的账号密码**（或让她本人在场），别升到一半卡在登录门
2. 万一弹出钥匙串授权框（「mcn-ai 想要访问钥匙串中的密钥」），点 **「始终允许」**
3. 真要重登也不丢东西：聊天记录在云端 + 本地 electron-store 双份，
   知识库是本地 md 文件，key 登录后自动下发（`login-provision` 实测 827ms 拿到两把）

> 记这条的理由：**乐观的结论最容易在别人机器上翻车**。实测说「不用重登」，
> 而准备工作按「可能要重登」做——两者不冲突，成本只是多带一个密码。

### M-29 冷调用：这一轮**没测出结论**（如实记）

HANDOFF §4-18 记着「根因是 ad-hoc 签名，签名后要复测一次再决定要不要上助手进程」。
复测跑了（`e2e/probe-safestorage.mjs`），但**新旧包都是 0ms，量不出差异**——
securityd 的签名校验缓存在连续跑了一小时构建与走查之后已经烘热，冷热差异在这种状态下
测不到。

**要拿到真正的冷态数字，得重启机器之后把这个探针当成第一件事跑**：

```bash
cd desktop && MCNAI_APP_BIN="$PWD/release/mac-arm64/mcn-ai.app/Contents/MacOS/mcn-ai" node e2e/probe-safestorage.mjs
```

判据写在脚本末尾：第 1 次（冷）的加密耗时若是个位数～几百 ms，说明签名解决了、
§4-18 那个「常驻助手进程」的方案可以正式作废；若仍是几秒～几十秒，那个方案要重新捡起来。
**在拿到重启后的数字之前，不要宣称「签名解决了 M-29」。**
...
## 0b. 这一版的检索口径 = 本地（2026-08-19 裁决）

`search_knowledge` **登录态下也走本地全文检索**，云端语义检索本版不启用。
原因：云端那一支不返回 `file_path`，模型拿不到可以直接 Read 的路径，只能从正文摘录里猜——
猜错 → Read 失败 → 再猜 → 再失败 → 只好 Grep 找（步骤流上就是连着两条「（这一步没成功）」）。
详见 HANDOFF §3-13。
...
| | |
|---|---|
| **开关在哪** | `~/Library/Application Support/mcn-ai-desktop/config.json` 的 `"searchBackend"`，出厂 `"local"` |
| **怎么切回云端** | 把它改成 `"cloud"`，**改完立刻生效，不用重启**。云端 RPC 修好之前不要切 |
| **云同步没有关** | 入库照常上云、embedding 照常写。**只是"查"暂时不走它**——云端修好后切回来即刻有完整数据可查，不用补录 |

> 对客户的说法：检索能力这一版走本地全文（快、给得出出处），云端语义检索在做，
> 不影响资料上云与将来的检索质量。
...
| 跑法 | 覆盖 | 耗时 | 花费 |
| --- | --- | --- | --- |
| `npm run verify -- --fast` | L0–L2（类型/IPC 契约/逻辑 smoke/pipeline 冻结产物） | ~20 秒 | 0 |
| `npm run verify` | 上面 + L3 全部 GUI 走查 | ~5 分钟 | 0 |
| `npm run verify -- --paid` | 上面 + L4 真实 AI 调用 | ~8 分钟 | ≈¥1 |

**发版必须至少跑到 `npm run verify`（不带参数）全绿。** 改过模型/线路再单独加
`npm run smoke:provider`（它要真实 key，不在总控里）。

> 为什么有这个总控：2026-08-19 真人原话「还是这样一个一个的试，太慢了」。
> 之前是手动一条条跑、停在第一个红上，三轮走查 45 分钟只暴露一个问题。

### 2b. 人工自测清单（自动化测不到的部分）

自动走查能证明"点了有反应、文件真落地"，但下面这些**只有人眼能判**——
装好包之后照这张单子过一遍，**逐条打勾**，有一条不对就别发。

**装机（3 分钟）**

- [ ] dmg 双击直接打开，**没有**"无法验证开发者"弹窗，**不需要**右键→打开
- [ ] Dock 图标、Finder 里的名字、菜单栏左上角，三处都是 **SamePage**
- [ ] 设置页底部 / 侧栏身份行显示的版本号 = 你这次发的版本号
      （0.1.0 就是栽在这条上：界面写死 v0.1.0，装了 0.1.1 还显示旧号）

**基本可用（5 分钟）**

- [ ] 用真实账号登录一次，能进主界面
- [ ] 新建对话 → 问一句 → 有回答；**中途点停止**能真停下
- [ ] 关掉软件再打开，**上一轮对话还在**（会话没丢）
- [ ] 检索框搜一个你知道库里有的词，**第一次搜就有结果**（不是搜第二次才有）

**知识库（5 分钟）**

- [ ] 拖一个文件进窗口 → 投递箱有反应 → 等它跑完 → 文件树里能找到
- [ ] 右键文件夹 → 新建笔记 / 新建文件夹 / 重命名 / 删除，四项都真生效
- [ ] 上传一个 docx/pdf 当**附件**提问，AI 能读到内容；这份文件**不该**进知识库
- [ ] 让 AI 改一篇笔记 → **弹确认卡**（有路径、有改动摘要、有允许/不允许）
- [ ] 允许之后 → 出撤销提示 → **点撤销 → 文件真的回到改之前**

**升级路径（发新版时必测）**

- [ ] 老版本装着的机器上，等它自己提示更新（或退出再开）→ 升上来之后
      **不用重新登录、知识库还在、历史对话还在**

> 换签名会不会强制重新登录：本机实测**不会**（升级走查 7/7 绿，会话真解出来了）。
> 但 Keychain 的授权记录是**每台机器各自**的，上门升级还是要做好可能要重登一次的准备。
