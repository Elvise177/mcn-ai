...
### H-07 投递箱正在跑时，在工作台/设置页完全没有出口 ✅ 2026-08-16 已修（任务层一期，侧栏底部 TaskDock）
...
### H-08 跑到一半切回知识库页：进度条不见了，看着像卡死 ✅ 2026-08-16 已修（任务层一期）
...
### H-09 点「停止生成」，已经流出来的半截回答直接消失 ✅ 2026-08-16 已修（任务层二期）
...
### H-10 生成中切走对话，回来后没有"进行中"状态，还能重复发送 ✅ 2026-08-16 已修（一期看得见 + 二期拒绝重发）
...
### H-13 投递箱任务开跑后无法取消 ✅ 2026-08-16 已修（任务层二期，杀进程组 + before-quit 清理）
...
**M-27 编辑中外部改动会被静默覆盖，而「Obsidian 同时编辑」是宣传卖点** ✅ 2026-08-16 已修（非模态提示条 + 保存三选一，默认「另存为副本」）
...
| P1（本轮迭代） | ~~H-06 ~ H-13~~ ✅ 全部完成（H-06 随 P0 批次；H-07/H-08 一期；H-09/H-10/H-13 二期；H-11/H-12 见下）＋ M-01 ✅ / M-03 ✅ | 异步三态与跨页面状态出口，已按「全局任务状态层」两期做完。**剩下 H-11（搜索空结果三态）、H-12（建库向导 busy 卡死）、M-02/M-04~M-09 未动**——设计 §0 明确它们是页面局部状态问题，与任务层无关，各自单独修更便宜 |
| P2 | M-10 ~ M-28（**M-27 已在二期做掉**） | 状态覆盖与一致性，可随各页面改动顺手带上 |
...
**T-01 `agentDrafts` 落盘 + 重启后把未完成回答补成一条中断消息**（低优先级）
...
✅ **2026-08-18 已修，本条销账**（`src/main/agent/index.ts` 的 result 分支）。
原始描述：SDK 出错时先吐一条 `result` 消息（内容就是英文报错，如 `Failed to authenticate. API Error: 401 …`），我们把它当成正常回答落进对话；紧接着 `for await` 抛出，又落一条 `⚠️ Error: …`。用户看到的是同一件事说两遍，且第一条是纯英文技术文案（截图 41c/41d 里看得很清楚）。

**分两次做完的**：
- 第一半随「会话恢复失败自动降级」那单（`8d23345`）：`subtype !== 'success'` 的错误型 result 不再落普通气泡。
  那单的直接病因是 `error_during_execution`——正是 T-02 的一个实例
- 第二半（本条销账那次）：**`subtype: 'success'` 但 `is_error: true`** 的那一支。
  上游 401/403/额度不足长的就是这个样子，`result` 字段里装着英文原文。它与前一支同等对待——
  扣住不发，最终走 `kind:'error'`（过 `lib/err.ts` 的 `zhError` 出中文 + 气泡里有「重试」），
  **英文原文只进 `logs/main.log`**

**判据上留了一处刻意的不对称**，别顺手"统一"掉：显示只认 `is_error`，**不认** `api_error_status`。
后者在用量记账那边的语义是"存疑就别计费"（宁可少记一条，代价为零），搬到显示上却会把一条
**真回答**判成错误、连正文一起丢掉——两边的容错方向正好相反。

**范围核对**：T-02 描述的就是"一次失败两条气泡 + 英文原文"，两支都收口了，**无余量**。
与之相邻但**不属于** T-02、仍然成立的既有行为：`zhError` 匹配不到的英文原文仍会原样显示并附
「可把这句发给管理员排查」——那是 B-5 拍板的规则（猜着翻比不翻更危险），不是本条的遗留。

**走查断言**：45c/41 那一步——一次失败在对话里**有且只有一条** assistant 消息、必须带
`error` 标记、且正文里不许出现连续 5 个以上英文字母（本地模式走预检 bail、`E2E_CHAT` 走 401 桩，
两种模式同一条断言）。另加一条爆炸半径守卫：成功那一轮必须有 `assistant` 事件、不许有 `error`
事件、且真的调过工具。
...
**B-8 检索器的分词不切中英文边界**（低优先级，2026-08-18 第 3 批走查发现，未修）

`src/main/vault/search-worker.ts` 的 `tokenize()` 与 `queryFragments()` 都用 `split(/[^\p{L}\p{N}]+/u)` 分段。
中文和英文字母都属于 `\p{L}`，所以**中英混排的连续串会被当成一整段**——
`zzz这个词库里一定没有zzz` 是一个 15 字的段，而不是 `zzz` + `这个词库里一定没有` + `zzz`。

**这次没有造成真实伤害**：它只影响 B-1 模糊回退里的「连续片段」判定，而
① 真实短查询（灰太太/星母/收支/绩效…共 14 个实测）全部走**精确**那一遍，根本不进模糊回退；
② 真·生造词（`zzqx月半仚`/`qwertyuiop`）仍然零命中，「没找到」这个状态可达。
发现它是因为走查里那个"生造词"其实是常用词拼的句子，模糊回退正常地捞到了一条
（`OMG美妆x向日花年框合作.md`——34 万字的表格文档撞上一个 3 字子串纯属概率）。

**为什么今天不修**：当时试过「片段长度随查询长度上升」，但 `min` 按整条查询的汉字数算、
而分段长度按段算，两个量对不上，收紧**实际未生效**；继续调阈值等于为一个测试构造物
调产品参数，每调一次都要重跑整套验证。2026-08-18 拍板：回滚该编辑，不留"看起来在收紧
实际没作用"的死代码，改记本条待办。

**将来怎么修**：分段时把中英边界切开（`\p{Script=Han}` 与 `[a-z0-9]` 分别成段），
片段长度门槛按**每段自身长度**分档（如段长 ≥10 时要求 ≥5 字片段），而不是按整条查询算。
**触发条件**：真实用户查询里出现中英混排长串并捞出不相关结果时。
修完必须重跑 `walkthrough.mjs` 里的 B-1 回归那 9 条 + 零命中态那步。
